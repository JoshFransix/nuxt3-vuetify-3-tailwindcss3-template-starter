<template>
    <div>
        <div v-if="!isLoading" class="tw-h-full tw-w-full">
            <v-form v-model="valid" class="sign-up-form" ref="form" @submit.prevent="$router.push('/dashboard')">
                <div class="tw-text-center tw-mb-12">
                    <h3 class="tw-text-white tw-mb-6 tw-text-2xl tw-font-semibold">
                        Welcome Back
                    </h3>
                    <span class="tw-text-[#ddd]">Please enter your details to log in.</span>
                </div>
            ...
            <h2 class="tw-text-sm tw-text-[#a1a1a1] tw-mt-4">
                ...
                <span @click="$emit('signup')" class="tw-text-brand-primary tw-cursor-pointer">Sign up?</span>
            </h2>
        </div>
    </div>
</template>
